...
本プロジェクトでは、民泊サービスの物件データを使用して、宿泊価格を予測するモデルを構築しました。データの前処理、特徴量エンジニアリング、複数の機械学習モデルの比較を通じて、高精度の予測モデルを作成しました。実際の民泊宿泊価格予測では、RMSE 147.97 を達成し、上位 16% にランクインしました。
